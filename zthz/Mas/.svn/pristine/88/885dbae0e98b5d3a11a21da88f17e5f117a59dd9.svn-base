...
import cn.zthz.tool.user.AProfile;
...
//			putError(request, response, "update user failed! " + e.getMessage(), ErrorCodes.SERVER_INNER_ERROR);
	...
	public void infop(HttpServletRequest request, HttpServletResponse response) {
		// checkUserToken(request, response);
		String userId = request.getParameter("userId");
		if (StringUtils.isEmpty(userId)) {
			putError(request, response, "user id is empty", ErrorCodes.PARAMETER_INVALID);
		...
		if (null != user) {
			...
			result.put("allowPush", user.getAllowPush());
			result.put("birthday", user.getBirthday());
			result.put("email", user.getEmail());
			result.put("telephone", user.getTelephone());
			result.put("mainPictureMid", user.getMainPictureMid());
			result.put("rank", user.getRank());
...
//	public void applyAuth(HttpServletRequest request, HttpServletResponse response){
//		String userId = request.getParameter(USER_ID);
//		try{
//			QuickDB.update("update User set applyAuth=1 where id='"+StringEscapeUtils.escapeSql(userId)+"'");
//			putSuccess(request, response);
//		}catch(Exception e){
//			putError(request, response, e);
//		}
//	}
	...
	public void applyAuth(HttpServletRequest request, HttpServletResponse response){
		String userId = request.getParameter(USER_ID);
		String name = getString(request, response, "name", true);
		Integer sex = getInt(request, response, "sex", true);
		String company = getString(request, response, "company", false);
		String address = getString(request, response, "address", false);
		String idcard = getString(request, response, "idcard", true);
		String reason = getString(request, response, "reason", true);
		String telephone = getString(request, response, "telephone", true);
		AProfile aProfile = new AProfile();
		aProfile.setUserId(userId);
		aProfile.setName(name);
		aProfile.setSex(sex);
		aProfile.setCompany(company);
		aProfile.setAddress(address);
		aProfile.setIdcard(idcard);
		aProfile.setTelephone(telephone);
		aProfile.setReason(reason);
		try{
